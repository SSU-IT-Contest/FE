...
const ModeSelector = ({ activeMode, setActiveMode, targetAudience, setTargetAudience, questionText, setQuestionText }: { activeMode: SummarizeMode; setActiveMode: (mode: SummarizeMode) => void; targetAudience: string; setTargetAudience: (style: string) => void; questionText: string; setQuestionText: (text: string) => void }) => {
  ...
  return (
    <div className="w-full">
      ...
      <div className="hidden md:flex w-full gap-2 md:gap-3 relative" data-tour="mode-buttons">
        {modes.map((mode) => (
              ...
              <button
                onClick={() => {
                  if (canUseFeature("summarize", "targeted")) {
                    ...
                    alert(`${getRequiredPlanName("summarize", "targeted")} 플랜부터 사용 가능합니다`);
                  }
                }}
                disabled={!canUseFeature("summarize", "targeted")}
                className={clsx("block w-full px-3 py-1.5 text-left text-xs whitespace-nowrap transition-colors border-t border-purple-200", canUseFeature("summarize", "targeted") ? "hover:bg-purple-100" : "text-gray-400 cursor-not-allowed opacity-50", activeMode === "타겟 요약" && "bg-purple-100 font-semibold")}>
                타겟 요약
              </button>
            </div>
          )}
